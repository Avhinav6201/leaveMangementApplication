...
export class UserServicesService {
  ...
  constructor(
    ...
    private encryption: EncryptionDecryptionService
  ) {}
  ...
  async login(email: String, password: String) {
    ...
    const encryptedresponse = await lastValueFrom(
      this.http.post(`${this.apiUrl}/login`, { encryptedPayload })
    );
  ...
  otpGenration(email: string): Observable<any> {
    return this.http
      .get<any>(`${this.apiUrl}/genrateOtp`, {
        params: { email },
      })
      .pipe(
        map((response) => {
  ...
  async ResetPassword(password: String) {
    ...
    const encryptedPayload = this.encryption.encryptData(
      JSON.stringify(payload)
    );
    console.log(payload);

    const encryptedresponse = await lastValueFrom(
      this.http.post(`${this.apiUrl}/resetPassword`, { encryptedPayload })
    );
  ...
  async registerForLeave(
    ...
    const encryptedresponse = await lastValueFrom(
      this.http.post(`${this.apiUrl}/registerForLeave`, { encryptedPayload })
    );
  ...
  async getLeavedetails() {
    const encryptedresponse = await lastValueFrom(
      this.http.get(`${this.apiUrl}/getLeavedetails`)
    );
  ...
  async getUserLeavedetails() {
    const encryptedresponse = await lastValueFrom(
      this.http.get(`${this.apiUrl}/getUserLeavedetails`)
    );
  ...
  async getUserLeave() {
    const encryptedresponse = await lastValueFrom(
      this.http.get(`${this.apiUrl}/getUserLeave`)
    );
  ...
  async validateToken(token: String, user_id: number) {
    ...
    const encryptedresponse = await lastValueFrom(
      this.http.post(`${this.apiUrl}/validatetoken`, { encryptedPayload })
    );
  ...
  async sendEmail(email: String) {
    ...
    const encryptedresponse = await lastValueFrom(
      this.http.post(`${this.apiUrl}/sendEmail`, { encryptedPayload })
    );
  ...
  async uploadcsv(formData: FormData) {
    const encryptedresponse = await lastValueFrom(
      this.http.post(`${this.apiUrl}/uploadcsv`, formData)
    );
