...
export class DashboardComponent implements OnInit {
  constructor(private dialog:MatDialog,private _userService:UserServicesService){
  ...
  iterateLeaveDetails() {
    ...
    for (let i = 0; i < this.userleaveDetails.length; i++) {
      if(this.userleaveDetails[i].leave_type_id===1){
       ...
       this.sick_percentage=((this.sick_used)/(this.sick_used+this.sick_available))*100
       ...
       this.Earned_percentage=((this.Earned_used)/(this.Earned_used+this.Earned_available))*100
       console.log(this.sick_percentage)
      }
      else if(this.userleaveDetails[i].leave_type_id===2){
       ...
       this.Adjustment_percentage=((this.Adjustment_available)/(this.Adjustment_available+this.Adjustment_percentage))*100
       ...
       this.unpaid_percentage=((this.unpaid_available)/(this.unpaid_available+this.unpaid_used))*100
       ...
       this.half_leave_percentage=((this.half_leave_available)/(this.half_leave_available+this.half_leave_used))*100
       ...
       this.casual_percentage=((this.casual_available)/(this.casual_available+this.casual_used))*100
