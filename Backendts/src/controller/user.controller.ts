...
class UserController {
  ...
  public async logIn(req: Request, res: Response) {
    ...
    try {
      ...
      console.log(error);
      return encrypt(
        JSON.stringify({ success: false, message: "Error in login." })
      );
    }
  }
    ...
    try {
      ...
      if (user.rows.length > 0) {
        ...
        return encrypt(
          JSON.stringify({
            success: false,
            message: "please do the signup process",
          })
        );
      }
    } catch (error) {
  ...
  public async verifyToken(req: Request, res: Response) {
    ...
    try {
      const dbtoken = await userModel.getResetPasswordToken(
        decryptedObj.user_id
      );
      if (
        dbtoken.rows.length > 0 &&
        dbtoken.rows[0].token === decryptedObj.token
      ) {
        ...
        return encrypt(
          JSON.stringify({
            success: false,
            message:
              "link expired please resend the email to reset the password",
          })
        );
      }
    } catch (err: any) {
      ...
      return encrypt(
        JSON.stringify({
          success: false,
          message: "link expired please resend the email to reset the password",
        })
      );
    }
  }
  ...
  public async resetPassword(req: Request, res: Response) {
    ...
    try {
      ...
      if (result.success && result.rowCount > 0) {
        ...
        const deletedtoken = await UserModel.deleteResetPasswordToken(
          this.user_id
        );
        ...
        return encrypt(
          JSON.stringify({
            success: false,
            message: "please do signup process.",
          })
        );
      }
    } catch (error) {
  ...
  public async registerForLeave(req: Request, res: Response) {
    ...
    const decryptedObj = decrypt(encryptedPayload);
    try {
      const result = await UserModel.checkforleave(
        req.user.id,
        decryptedObj.leaveType
      );
      ...
      if (result.success) {
        if (leave == null || undefined) {
          ...
          const newAvailable =
            Number(leave.available) - Number(decryptedObj.noOfDays);
          const UpdatedUsed =
            Number(leave.used) + Number(decryptedObj.noOfDays);

          const Updatedresult = await UserModel.updateUserLeave(
            req.user.id,
            decryptedObj.leaveType,
            UpdatedUsed,
            newAvailable
          ...
          if (Updatedresult.success) {
            const result = await UserModel.userleaveRequest(
              req.user.id,
              decryptedObj.startDate,
              decryptedObj.endDate,
              decryptedObj.leaveType,
              decryptedObj.noOfDays,
              decryptedObj.reason
            );
            if (result.rowCount > 0) {
              ...
              return encrypt(
                JSON.stringify({
                  success: false,
                  message: "some error occurred during the updation",
                })
              );
            }
          } else {
          ...
          return encrypt(
            JSON.stringify({
              sucess: true,
              message: "Currently you are not having available Leave",
            })
          );
        }
      }
    ...
    try {
      ...
      if (result.success) {
        ...
        return encrypt(
          JSON.stringify({
            success: false,
            message: "not able to fetch leaveType",
          })
        );
      }
    } catch (err) {
    ...
    try {
      ...
      if (result.success) {
        ...
        return encrypt(
          JSON.stringify({
            success: true,
            message: "user leave details fetched",
            result,
          })
        );
      } else {
  ...
  async upload(req: Request, res: Response) {
    try {
      if (req.file) {
        ...
        .on('end', async () => {
           for(let i=0;i<results.length;i++){
            let leaveExist=await userModel.CheckuserleavewithLeaveType(results[i].leave_name,results[i].user_email)
            if(leaveExist.rows.length>0){
            
            await userModel.updateuserLeave(results[i].available,results[i].used,results[i].user_email,results[i].leave_name,this.user_id)
            }else{
              await userModel.insertUserLeave(results[i].user_email,results[i].leave_name,results[i].available,results[i].used,this.user_id)
            }
           }
        })
        .on('error', (error) => {
        ...
        return encrypt(JSON.stringify(
          {
            success:false,message:'file not uploaded please try again'
          }
        ))
      ...
      return encrypt(JSON.stringify({
        success:false,message:'some error occurred during the uploading the file please try again later'
      }))
